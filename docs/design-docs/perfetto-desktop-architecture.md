...
| `0006-skip-test-data-with-env.patch` | Make the unconditional `//test/data` sync inside `tools/install-build-deps` opt-in via `PERFETTO_SKIP_TEST_DATA=1` | **Landed 2026-05-10.** The desktop wrapper only builds the UI; Perfetto's diff/integration test fixtures are dead weight that adds 5+ minutes of cold-cache CI time. `scripts/setup.sh` exports the env var when invoking `install-build-deps --ui`. |
